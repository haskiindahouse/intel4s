...
```
  ...
  3. Scalameta parse (parallel)
     ...
     │  - Parse source → AST (Scala 3 dialect)
...
```bash
# macOS Apple Silicon
curl -fsSL https://github.com/nguyenyou/scalex/releases/latest/download/scalex-macos-arm64 -o /usr/local/bin/scalex && chmod +x /usr/local/bin/scalex

# macOS Intel
curl -fsSL https://github.com/nguyenyou/scalex/releases/latest/download/scalex-macos-x64 -o /usr/local/bin/scalex && chmod +x /usr/local/bin/scalex

# Linux x64
curl -fsSL https://github.com/nguyenyou/scalex/releases/latest/download/scalex-linux-x64 -o /usr/local/bin/scalex && chmod +x /usr/local/bin/scalex
```
...
cp scalex /usr/local/bin/scalex
```

## Claude Code Integration

Install from the marketplace — Claude automatically uses scalex for Scala navigation:
...
Or use directly during a session:

```bash
claude --plugin-dir /path/to/scalex/plugin
```

The plugin includes a [skill](plugin/skills/scalex/SKILL.md) that teaches Claude when and how to call each command.
