...
class IndexSuite extends ScalexTestBase:
  ...
  test("index persists to disk and reloads") {
    ...
    idx1.index()
    assert(idx1.parsedCount == 37, s"Cold index should parse all 37 files, got ${idx1.parsedCount}")

    // Second index — warm (all cached)
    val idx2 = WorkspaceIndex(workspace)
    idx2.index()
    assert(idx2.cachedLoad, "Second index should load from cache")
    assert(idx2.skippedCount == 37, s"Warm index should skip all 37 files, got ${idx2.skippedCount}")
  ...
  test("index re-parses changed files") {
    ...
    assert(idx2.skippedCount == 36, s"Should skip 36 files, got ${idx2.skippedCount}")
  }
